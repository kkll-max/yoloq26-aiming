...
# 主函数
def main(input_folder, output_folder, model_path, conf_threshold=0.65, nms_threshold=0.45):
    # 加载模型
    ...
    for filename in os.listdir(input_folder):
        ...
        # 预处理
        input_tensor, scale, dx, dy = preprocess(img)
        ...
        features = {}
        for out in outputs:
            batch_size, H, W, C = out.shape
            stride = 640 // H
            if stride not in features:
                features[stride] = {}
            if C == 4:
                features[stride]['box'] = out.reshape(-1, 4)
            elif C == 8:
                features[stride]['kpt'] = out.reshape(-1, 4, 2)
            else:
                features[stride]['cls'] = out.reshape(-1, C)
            
        # 后处理
        results = postprocess(features, img.shape, scale, dx, dy)
        
        # 绘制结果
        result_img = draw_results(img.copy(), results)
...
if __name__ == "__main__":
    input_dir = '/data/images'  # 替换为你的输入文件夹路径
    output_dir = 'image_output_onnx'
    model_file = 'model/auto_aim_modified.onnx'  # 替换为你的ONNX模型路径
    
    main(input_dir, output_dir, model_file)
